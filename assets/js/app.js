...
app.controller('ctrlConBull', ['$scope', function ($scope) {
  ...
  $scope.gerarPDF = function() {            
    ...
    doc.addImage(imgData2, 'JPEG', 150, 10, 40, 20);
  ...
  $scope.popular = function(doc){    
    ...
    var linha = 55;
    var coluna = [10,80,160];
    var _coluna = 0;
    
    doc.setFontSize(12);
    doc.text(10, 45, "Brinco");
    doc.text(80, 45, "Data de Nascimento");
    doc.text(160, 45, "Situacao");
    
    for(var i = 0; $scope.bois.length > i; i++){                
      doc.text(coluna[_coluna], linha, $scope.bois[i].brinco);
      _coluna++;    
      doc.text(coluna[_coluna], linha, "10/12/2014");
      _coluna++;                                                  
      doc.text(coluna[_coluna], linha, "Abatido");                  
